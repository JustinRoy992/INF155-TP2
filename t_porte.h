...
Description: Énumère les types de portes permis (AND, OR, NOT, XOR)
*/
...
/*
Fonction: PORTE_DESTROY
Description: Libère la mémoire occupée par la porte et tous ses composants.
NDE: Ne pas oublier de détruire toutes les t_pin_entree et le t_pin_sortie appartenant à la porte.
Paramètres:
...
/*
Fonction: PORTE_CALCULER_SORTIE
Description: Calcule la valeur du pin de sortie de la porte, selon la valeur des t_pin_entree 
			 et le type de porte. Par exemple, si la porte est une porte ET, et que les deux
			 t_pin_entree de la porte ont une valeur de 1, le t_pin_sortie de la porte doit
			 avoir la valeur 1. 
NDE: 
- N'oubliez pas qu'il existe des opérateurs binaires en C. Utilisez-les!
- N'oubliez pas que, pour modifier la valeur d'un t_pin_sortie vous devez utiliser le
  mutateur correspondant (par exemple : pin_sortie_set_valeur()) et ne pas modifier la valeur
  du champ directement ici (car nous sommes à l'extérieur du module t_pin_sortie).
Paramètres:
- porte: Pointeur vers la porte dont on souhaite calculer la sortie de signal. 
...
/*
Fonction: PORTE_RELIER
Description: Relie une des entrées de la porte à un autre composant du circuit 
             (une source ou la sortie d'une autre porte)
			 On note ici qu'on relie un des t_pin_entree de la porte vers le t_pin_sortie
			 d'un autre composant. Les seuls autres composants disposant d'une t_pin_sortie sont
			 les sources du circuit et les autres portes. 
...
- num_entree: l'indice de la t_pin_entree à relier dans le tableau des entrées. 
...
- source: Le t_pin_sortie vers laquelle nous allons relier l'entrée de la porte. 
...
/*
Fonction: PORTE_PROPAGER_SIGNAL
Description: Propage le signal à partir de la porte passée en paramètre.
			 ...
			 le signal à partir du t_pin_sortie de la porte.
NDE: Une fois que vous avez calculé la valeur du pin de sortie,  
	 utilisez la fonction pin_sortie_propager_signal().
...
*/
void porte_serialiser(const t_porte *porte, char * resultat);   //*** Partie 2
